...
def objectiveCounts_random(params, train=True):
    ...
    try:
        ...
        p1, p2, tau1, tau2,gain,max_rate,y,theta  = params
        ...
        for i in selected_indices:
            ...
            # profiler.start("spike_generation")
        ...
        # spike_distances_vp = np.zeros(len(spike_trains_modelvp))
        # for i in range(len(spike_trains_modelvp)):
        #     spike_distances_vp[i] = victor_purpura_distance([spike_trains_modelvp[i], reference_spike_trains[i]],q)[0,1]
...
bounds = [(1, 2), (0.1,1.5), (20,80),(80,200),(0.0001, 0.01), (50,400), (-900,4000),(5,140)] # Nonlinearity(0.001, 0.008), (220,380), (-900,-200),(5,140)
...
# Global variables to track best parameters based on evaluation score
BEST_EVAL_PARAMS = None
BEST_EVAL_SCORE = None
BEST_EVAL_ITERATION = None

def callbackF(xk, convergence):
    global BEST_EVAL_PARAMS, BEST_EVAL_SCORE, BEST_EVAL_ITERATION
    ...
    # Track best parameters based on evaluation score (lower is better)
    if BEST_EVAL_SCORE is None or eval_error < BEST_EVAL_SCORE:
        BEST_EVAL_PARAMS = xk.copy()
        BEST_EVAL_SCORE = eval_error
        BEST_EVAL_ITERATION = iteration
        print(f"Iteration {iteration}: Training Error = {train_error:.4f}, Evaluation Error = {eval_error:.4f} *** NEW BEST EVAL SCORE! ***")
    else:
        print(f"Iteration {iteration}: Training Error = {train_error:.4f}, Evaluation Error = {eval_error:.4f}")
...
if __name__ == '__main__':
    ...
    movies_2d  = movies_f32.reshape(n_trials*T, Y*X)   # view, no copy
    del movies_f32
    # pre-flatten your RF
    spatial_rf_flat = spatial_filterV3.reshape(Y*X).astype(np.float32)
    # do one batched spatial filter
    linear_flat = movies_2d @ spatial_rf_flat          # (~700*151,)
    # now reshape to per-trial
    GLOBAL_LINEAR = linear_flat.reshape(n_trials, T) 
    del movies_2d, linear_flat
    ...
    optimal_1, optimal_2,optimal_3,optimal_4,optimal_5,optimal_6,optimal_7,optimal_8 = result_de.x
    ...
    print(f"Final training score: {result_de.fun:.4f}")
    print(f"Best evaluation score: {BEST_EVAL_SCORE:.4f} (found at iteration {BEST_EVAL_ITERATION})")
    print(f"Best evaluation parameters: {BEST_EVAL_PARAMS}")
    ...
    results = {
        "optimal_params": optimal_params,  # Final training-based parameters
        "best_eval_params": BEST_EVAL_PARAMS,  # Best evaluation-based parameters
        "best_eval_score": BEST_EVAL_SCORE,  # Best evaluation score
        "best_eval_iteration": BEST_EVAL_ITERATION,  # Iteration where best eval was found
    ...
    with open("/projects/p32750/repository/Natural_motion-model/V3/results/V3_control_counts_withoutGC.pkl", "wb") as f:
        pickle.dump(results, f)

    print("Optimization results saved to V3_control_counts_withoutGC.pkl") 
