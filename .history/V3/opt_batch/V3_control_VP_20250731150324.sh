...
#SBATCH --job-name=V3_control_VP  ## When you run squeue -u NETID this is how you can identify the job
#SBATCH --output=V3_control_VP ## standard out and standard error goes to this file
...
echo "Starting V3 Control VP Optimization"
...
# Check exit status
if [ $? -eq 0 ]; then
    echo "V3 Control VP optimization completed successfully"
    echo "Results saved to: /projects/p32750/repository/Natural_motion-model/V3_control_VP_results.pkl"
else
    echo "V3 Control VP optimization failed with exit code $?"
    exit 1
fi
