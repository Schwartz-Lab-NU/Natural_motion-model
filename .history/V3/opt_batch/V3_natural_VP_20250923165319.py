...
# model PARAM
delay = 0.05 # s
t_sampling = np.linspace(0,91,91)
...
random.seed(42)
all_indices = list(range(1773))
random.shuffle(all_indices)
train_indices = all_indices[:886]
n =3
eval_indices = all_indices[886:]

spike_trains_n = []
spike_trains_nemd = []
for trial_idx, spikes in enumerate(spike_indices_n):
    st_n = SpikeTrain(spikes.flatten() / 10 * ms, t_stop=1500)
...
def objectiveCounts_random(params, train=True):
    ...
    try:
        ...
        for i in selected_indices:
            ...
            st_modelvp = SpikeTrain(np.array(np.where(spikingindx > 0)[0]).flatten() / 54 * 10 * ms,t_stop=15200/6)
...
def sigma_objective(sigma_array):
    ...
    def callbackF(xk, convergence):
        nonlocal BEST_EVAL_PARAMS, BEST_EVAL_SCORE, BEST_EVAL_ITERATION
...
if __name__ == '__main__':
    ...
    res = minimize(
        ...
        bounds=[(5.0, 100.0)],        # Adjust bounds as needed
        method='L-BFGS-B',
    ...
    results = {
        ...
        "optimal_params": BEST_DE_PARAMS,  # Final training-based parameters
    ...
    with open("/projects/p32750/repository/Natural_motion-model/V3/results/V3_natural_VP_results.pkl", "wb") as f:
        pickle.dump(results, f)

    print("Optimization results saved to V3_natural_VP_results.pkl") 
