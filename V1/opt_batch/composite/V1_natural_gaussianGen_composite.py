...
if stimttype == 'natural_gaussianGen':
    ...
    A = 1
    sigma_x = 27.24267611684103
    sigma_y = 27.24267611684103
    alpha = 0.5005570525249503
    T, X_SF, Y_SF = stimuli.shape 
    x_SF = np.linspace(-(X_SF-1)/2, (X_SF-1)/2, X_SF)
    y_SF = np.linspace(-(Y_SF-1)/2, (Y_SF-1)/2, Y_SF)
    spatial_rf = generalized_gaussian_2d(x_SF, y_SF, sigma_x, sigma_y, alpha, A=1.0, x0=0, y0=0)
...
def objectiveVP(params, train=True):
    try:
        sigma, B, tau, alpha, gain, max_rate, y, theta = params
        # For natural_gaussianGen stimulus type, we use generalized_gaussian_2d
        spike_trainGCvpfit = []
        ...
        # Use generalized_gaussian_2d with optimized parameters
        spatial_rf = generalized_gaussian_2d(x_SF, y_SF, sigma, sigma, alpha, A=1.0, x0=0, y0=0)
...
# Define bounds for sigma, B, tau, alpha, gain, max_rate, y, theta
bounds = [(10, 70), (0.00001, 0.05), (2, 25), (0, 1),
          (0.001, 0.01),   # gain
          (150, 400),      # max_rate
          (-600, 4000),
          (-200, 300)]       # theta
...
if __name__ == '__main__':
    ...
    optimal_sigma, optimal_B, optimal_tau, optimal_alpha, optimal_gain, optimal_max, optimal_y, optimal_theta = result_de.x
    ...
    with open("/projects/p32750/repository/Natural_motion-model/V1_natural_gaussianGen_composite.pkl", "wb") as f:
        pickle.dump(results, f)

    print("Optimization results saved to V1_natural_gaussianGen_composite.pkl") 