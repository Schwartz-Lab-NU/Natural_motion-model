...
q = 1.0 / (100.0 * pq.ms)# Load data and define fixed parameters.

# Load data and define fixed parameters.

def objectiveEMD(params, train=True):
    try:
        sigma_x, B, tau, alpha, gain, max_rate, y = params
        # Modify the receptive field using the pnratio.
        t_temporal = np.linspace(-2000/3,0, 40) 
        # reversed_temp_rf = biphasic_temporal_filter(t_temporal, p1, p2, tau1, tau2, 3)[::-1]
        sigma_y = sigma_x
        spike_trainGCfit = []
        ...
        x_SF = np.linspace(-(X_SF-1)/2, (X_SF-1)/2, X_SF)
        y_SF = np.linspace(-(Y_SF-1)/2, (Y_SF-1)/2, Y_SF)
        spatial_rf = generalized_gaussian_2d(x_SF, y_SF, sigma_x, sigma_y, alpha, A=1, x0=0, y0=0)
        LoutGCfit = linear_filter( spatial_rf, stimuli, reversed_temp_rfTemp)
        ...
        for st in spike_trains_modelemdfit:
            if len(st) == 0:
                return 1e10
            if len(st) > 500:
                return 1e10     
...
def objectiveVP(params, train=True):
    try:
        sigma, B, tau, gain, max_rate, y, theta = params
        # For natural_gaussian stimulus type, we use gaussian_2d
        spike_trainGCvpfit = []
        ...
        # Use gaussian_2d with optimized parameters
        ...
        for spikes in spike_trainGCvpfit:
            st_model = np.array(spikes).flatten() / 54 * 10
            spike_trains_model.append(st_model)
        # Select the appropriate real data trials.
        if train:
            real_spike_trains_VP = [spike_trains_n[i] for i in range(len(spike_trains_n)) if i % 2 == 0]
            real_spike_trains_EMD = [spike_trains_nemd[i] for i in range(len(spike_trains_nemd)) if i % 2 == 0]
        else:
            real_spike_trains_VP = [spike_trains_n[i] for i in range(len(spike_trains_n)) if i % 2 == 1]
            real_spike_trains_EMD = [spike_trains_nemd[i] for i in range(len(spike_trains_nemd)) if i % 2 == 1]
        
        # Combine the real and model-generated spike trains.
        spike_trainsvpfit = real_spike_trains_VP + spike_trains_modelvp
        spike_trains_EMD = real_spike_trains_EMD + spike_trains_model
        # Check for empty spike trains.
        ...
        EMD_matrix = compute_emd_matrix(spike_trains_EMD)
        score_EMD = block_means(EMD_matrix, 2, 30)
        ...
        score = score_EMD[0, 1] / score_EMD[0, 0] + score_VP[0, 1] / score_VP[0, 0]
        return score
...
# Define bounds for sigma, B, tau, gain, max_rate, y, theta
bounds = [(10, 70), (0.00001, 0.05), (2, 25),
          (0.001, 0.01),   # gain
          (150, 400),      # max_rate
          (-600, 4000),
          (-200, 300)]       # theta
...
if __name__ == '__main__':
    ...
    optimal_sigma, optimal_B, optimal_tau, optimal_gain, optimal_max, optimal_y, optimal_theta = result_de.x
    ...
    with open("/projects/p32750/repository/Natural_motion-model/V1_natural_gaussian_composite.pkl", "wb") as f:
        pickle.dump(results, f)

    print("Optimization results saved to V1_natural_gaussian_composite.pkl") 