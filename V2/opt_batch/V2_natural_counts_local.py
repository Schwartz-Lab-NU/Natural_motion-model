...
def objectiveCounts_random(params, train=True, stimuli=None, shape=None, dtype=None):
...
def callbackF(xk, convergence, stimuli, shape, dtype):
    # Record current iteration number.
    iteration = len(iterations) + 1
    # Compute errors on training and evaluation sets.
    profiler.start("callback_evaluation")
    train_error = objectiveCounts_random(xk, train=True, stimuli=movies_c_np, shape=shape, dtype=dtype)
    eval_error = objectiveCounts_random(xk, train=False, stimuli=movies_c_np, shape=shape, dtype=dtype)
...
if __name__ == '__main__':
    ...
    worker_func = partial(objectiveCounts_random, stimuli=movies_c_np, shape=shape, dtype=dtype)
    callback_func = partial(callbackF, stimuli=movies_c_np, shape=shape, dtype=dtype)
    ...
    with open("V2_natural_counts_test.pkl", "wb") as f:
        pickle.dump(results, f)

    print("Optimization results saved to V2_natural_counts_test.pkl") 