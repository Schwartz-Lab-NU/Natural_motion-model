...
def apply_temporal_filter_to_movie_gpu(movie_gpu, temporal_filter_gpu):
    """GPU version of temporal filter application"""
    F, = movie_gpu.shape
    T = len(temporal_filter_gpu)
    filtered_movie_gpu = cp.zeros_like(movie_gpu)
    
    # Use CuPy's convolve function
    filtered_signal_gpu = cp.convolve(movie_gpu, temporal_filter_gpu, mode='full')[:F]
    filtered_movie_gpu[:] = filtered_signal_gpu
    return filtered_movie_gpu
...
def linear_filter(spatial_rf, noise_input, temporal_filter):   
    ...
    if GPU_AVAILABLE:
        noise_input_gpu = cp.asarray(noise_input.astype(np.float32))
        spatial_rf_gpu = cp.asarray(spatial_rf.astype(np.float32))
        temporal_filter_gpu = cp.asarray(temporal_filter.astype(np.float32))
        
        # Perform matrix multiplication on GPU
        spatial_filtered_movie_gpu = cp.dot(noise_input_gpu.reshape((T, Y*X)), spatial_rf_gpu.reshape((Y*X)))
        
        # Apply temporal filter on GPU
        filtered_movie_gpu = apply_temporal_filter_to_movie_gpu(spatial_filtered_movie_gpu, temporal_filter_gpu)
        
        # Transfer result back to CPU
        if GPU_AVAILABLE:
            filtered_movie = cp.asnumpy(filtered_movie_gpu)
        else:
            filtered_movie = filtered_movie_gpu
    else:
        # CPU fallback
        noise_input = noise_input.astype(np.float32)
        spatial_rf = spatial_rf.astype(np.float32)   
        spatial_filtered_movie = noise_input.reshape((T,Y*X)) @ spatial_rf.reshape((Y*X))
...
def gain_control(Lout, B=0.005, tau=11.0):
    T = len(Lout)
    if GPU_AVAILABLE:
        Lout_gpu = cp.asarray(Lout.astype(cp.float32))
        decay_kernel_gpu = cp.asarray(B * cp.exp(-cp.arange(T) / tau), dtype=cp.float32)
        # Compute v_t as causal convolution (exclude current t)
        # Pad gain_controlled_Lout_gpu with 0 at start for correct causal convolution
        gain_controlled_Lout_gpu = cp.zeros(T, dtype=cp.float32)
        g_v_gpu = cp.zeros(T, dtype=cp.float32)
        # First, initialize gain_controlled_Lout_gpu[0]
        gain_controlled_Lout_gpu[0] = Lout_gpu[0]
        # Compute v_t for t >= 1 using convolution of gain_controlled_Lout_gpu
        # For the first pass, use Lout_gpu as an approximation (since g_v[0]=1)
        v_t_gpu = cp.convolve(gain_controlled_Lout_gpu, decay_kernel_gpu, mode='full')[:T]
        # Now, update g_v and gain_controlled_Lout in a single pass
        for t in range(1, T):
            g_v_gpu[t] = 1 if v_t_gpu[t] < 0 else 1 / (1 + v_t_gpu[t]**4)
            gain_controlled_Lout_gpu[t] = g_v_gpu[t] * Lout_gpu[t]
        # Recompute v_t with updated gain_controlled_Lout_gpu if needed (usually one pass is enough)
        # Transfer results back to CPU
        gain_controlled_Lout = cp.asnumpy(gain_controlled_Lout_gpu)
        g_v = cp.asnumpy(g_v_gpu)
    else:
...
# Load data
with open('results/spike_indices_nV2.pkl', 'rb') as f:
    spike_indices_n = pickle.load(f)
with open('results/spike_indices_cV2.pkl', 'rb') as f:
...
def objectiveCounts_random(params, train=True):
    try:
        ...
        # For control stimulus type, use spatial_filterSF with control receptive field
        ...
        reference_spike_trains = [spike_trains_c[i] for i in selected_indices]  # Use control spike trains
        profiler.end("reference_spike_trains")

        profiler.start("stimuli_loading")
        # Load all stimuli data first
        with h5py.File('stimuli400_cV2.h5', 'r') as f:  # Use control stimuli
            movies_c = f['stimuli']  # Convert to numpy array
...
# Define bounds for B, tau, gain, max_rate, y, theta (control stimulus type)
...
if __name__ == '__main__':
    ...
    with open("results/V2_control_counts.pkl", "wb") as f:
        pickle.dump(results, f)

    print("Optimization results saved to V2_control_counts.pkl") 