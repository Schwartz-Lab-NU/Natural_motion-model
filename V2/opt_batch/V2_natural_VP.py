...
rfN = np.load('/projects/p32750/repository/Natural_motion-model/rfN_V2.npy')
rfN[rfN < 0.1] = 0
spatial_filterV2 = rfN
...
if __name__ == '__main__':
    ...
    npy_path = '/projects/p32750/repository/Natural_motion-model/V2_stimuli_n.npy'
    movies_f32 = np.load(npy_path, mmap_mode='r').astype(np.float32, copy=False)
    n_trials, T, Y, X = movies_f32.shape
    movies_2d  = movies_f32.reshape(n_trials*T, Y*X)   # view, no copy
    del movies_f32
    # pre-flatten your RF
    spatial_rf_flat = spatial_filterV2.reshape(Y*X).astype(np.float32)
    ...
    with open("/projects/p32750/repository/Natural_motion-model/V2_natural_VP_multicore50.pkl", "wb") as f:
        pickle.dump(results, f)

    print("Optimization results saved to V2_natural_VP_multicore.pkl") 
