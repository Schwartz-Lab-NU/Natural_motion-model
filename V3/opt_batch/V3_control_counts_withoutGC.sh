...
#SBATCH --job-name=V3_control_counts_withoutGC  ## When you run squeue -u NETID this is how you can identify the job
#SBATCH --output=V3_control_counts_withoutGC ## standard out and standard error goes to this file
...
echo "Starting V3 Control counts Optimization"
...
python V3_control_counts_withoutGC.py

# Check exit status
if [ $? -eq 0 ]; then
    echo "V3 Control counts optimization completed successfully"
    echo "Results saved to: /projects/p32750/repository/Natural_motion-model/V3/results/V3_control_counts_withoutGC.pkl"
else
    echo "V3 Control counts optimization failed with exit code $?"
    exit 1
fi
